...
clean_subset <- function(data, region, phase, treatment1, scode, y_imp){
  ...
  #change shrub heights of 0 to NA so that they get removed when calculating quantiles, means
  data$height_shrub_ARTRW8[data$height_shrub_ARTRW8 == 0] <- NA
  data$height_shrub_CHVI8[data$height_shrub_CHVI8 == 0] <- NA
  data$height_shrub_PUST[data$height_shrub_PUST == 0] <- NA
...
data <- clean_subset(data, region = 'JP', phase = '2', treatment1 = 'ME', scode = scode, y_imp = y_imp)
