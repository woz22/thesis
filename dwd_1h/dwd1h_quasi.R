...
mast <- read.csv('C:\\Users\\User\\Documents\\GitHub\\thesis\\dwd_1h\\mastication.ed2.csv', header = TRUE)
...
coef(mlm)
mquasi <- glm(kgha_100h_1000h ~ TC + time + TC:time, 
              family = quasi(link = "identity", variance = "mu"), data = mast, 
              start = c(coef(mlm)))
