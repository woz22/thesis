...
####1. Estimate an AFT model with stress as a quantitative explanatory variable and specifying a weibull distribution. Report parameter estimates, and produce plots of survival, hazard, and probability density functions. Interpret the effect of stress on the AFT model, and on the hazard function.

#####Multiplicative effect of stress based on the AFT model: 

For each 100 unit increase in stress, the number of thousands of cycles until failure is compressed by a factor of 0.1525.

#####Multiplicative effect of stress on hazard function: 

For each 100 unit increase in stress, the hazards increase by factor of 21.4.
...
p <- p + geom_line(aes(linetype = factor(stressquant)))
p <- p + labs(x = "Thousands of Cycles", 
              y = "Probability", 
...
```{r}
springs$stress <- factor(springs$stress, 
                         levels = c('700', '750', '800', '850', '900', '950'))
...
p <- ggplot(d, aes(x = time, y = est))
p <- p + geom_line(aes(linetype = stress))
p <- p + labs(x = "Thousands of Cycles", 
              y = "Probability", 
              title = 'Survival Functions', 
...
#for each location estimate discrete marginal effect of RC length 
#between 10 and 15, and 15 and 20 mm
trtools::margeff(m, 
...
m <- vglm(cbind(immature, mature, ripe) ~ midpoint + sex, 
          family = cratio, data = mysturgeon)
...
p <- ggplot(d, aes(x = midpoint, y = probability, linetype = maturity))
p <- p + geom_line() + ylim(0,1) + theme_bw() + facet_wrap(~sex)
p <- p + labs(x = 'Size Midpoint (cm)', y = 'Probability', linetype = 'Maturity')
plot(p)
...
m <- vglm(cbind(immature, mature, ripe) ~ midpoint + sex, 
          family = propodds, data = mysturgeon)
cbind(coef(m), confint(m))
...
m <- vglm(cbind(immature, mature, ripe) ~ midpoint + sex, 
          family = multinomial(refLevel = "immature"), data = mysturgeon)
cbind(coef(m), confint(m))
