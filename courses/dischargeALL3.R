...
pn <- pn + geom_line(aes(y = discharge_pre, color = '#00BFC4'))
pn <- pn + geom_line(aes(y = discharge_post, color = '#F8766D'))
pn <- pn + scale_color_discrete(name = "", labels = c("Post-Fire (2013-14)","Pre-Fire (2012-13)"))
pn <- pn + theme_bw(base_size = 16)
pn <- pn + scale_x_date(date_breaks = "1 month", date_labels =  "%b") 
pn <- pn + scale_y_continuous(limits = c(0,2500), breaks = seq(0, 2500, by = 500), expand = c(0,0))
...
pa <- pa + scale_y_continuous(limits = c(0,2500), breaks = seq(0, 2500, by = 500), expand = c(0,0))
...
pn2 <- ggplot(data, aes(x = date.x))
pn2 <- pn2 + geom_line(aes(y = discharge_pre), color = '#F8766D')
pn2 <- pn2 + geom_line(aes(y = discharge_post), color = '#00BFC4')

pn2 <- pn2 + theme_bw(base_size = 16)
pn2 <- pn2 + scale_x_date(date_breaks = "1 month", date_labels =  "%b") 
pn2 <- pn2 + scale_y_continuous(limits = c(0,2500), breaks = seq(0, 2500, by = 500), expand = c(0,0))
pn2 <- pn2 + theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 18))
pn2 <- pn2 + labs(colour = "Year", title = 'Discharge of South Fork Boise River', subtitle = 'Neal Bridge stream gage',
                x = '', y = expression('Discharge (ft'^{3}*'/sec)'))
