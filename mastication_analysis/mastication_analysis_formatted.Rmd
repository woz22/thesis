...
##Model
```{r}
m <- lmer(sqrt(dwd_1hr) ~ pre_tree_cvr + yst + pre_tree_cvr:yst + (1|site), data = d)
```
...
##Model
```{r}
m <- lmer(sqrt(dwd_10hr) ~ pre_tree_cvr + yst + pre_tree_cvr:yst + (1|site) + (1|subplot_id), data = d)
```
...
##Model
```{r}
m <- lmer(sqrt(dwd_100_1000hr) ~ pre_tree_cvr + yst + pre_tree_cvr:yst + (1|site) + (1|subplot_id), data = d)
```
...
##Model
```{r}
m <- lmer(sqrt(duff) ~ yst + pre_tc + yst:pre_tc + (1|site) + (1|subplot_id), data = d)
```

```{r, echo = FALSE}
d$yhat_duff <- predict(m, re.form = NA)
p <- ggplot(data = d, aes(x = pre_tc, y = sqrt(duff)))
p <- p + geom_jitter()
...
p <- ggplot(data = d, aes(x = pre_tc, y = sqrt(duff), color = site))
p <- p + geom_jitter()
p <- p + geom_line(aes(y = yhat_duff))
...
Tree Dominance Index (TDI) = (pre-treatment tree cover)/(pre-treatment tree cover + pre-treatment grass cover + pre-treatment shrub cover)
...
##Model
```{r}
m <- lmer(sqrt(herb_ttl) ~ TDI + yst + yst:TDI + (1 + yst|site), data = l)
...
m <- lmer(sqrt(shrub_fuel) ~ TDI + yst + yst:TDI + (1 + yst|scode), data = l2)
summary(m)
lincon(m)
...
p <- ggplot(data = l2, aes(x = TDI, y = sqrt(shrub_fuel), color = scode))
p <- p + geom_point()
p <- p + geom_line(aes(y = yhat_shrub))
p <- p + theme_bw()
p <- p + labs(title = 'Shrub Fuels',
...
```{r}
m <- lmer(sqrt(can_cover_pt_pgrass) ~ TDI + yst + yst:TDI + (1|scode) + (1|subplot_id), data = l)
summary(m)
lincon(m)
...
```{r}
m <- lmer(sqrt(can_cover_pt_agrass) ~ TDI + yst + yst:TDI + (1 + yst|scode), data = l)
summary(m)
lincon(m)
...
```{r}
m <- lmer(sqrt(tree_dns_gt50_JUOS + tree_dns_gt50_PIED) ~ 
            TC + factor(yst) + factor(yst):TC + (1 + factor(yst)|scode), data = td)
summary(m)
lincon(m)

#by yst
td$yhat_tree_dens <- predict(m)
p <- ggplot(data = td, aes(x = TC, 
                           y = sqrt(tree_dns_gt50_JUOS + tree_dns_gt50_PIED)))
p <- p + geom_point()
p <- p + theme_bw()
...
p <- p + facet_wrap(scode~yst, ncol = 6)
plot(p)
...
m <- lmer(sqrt(tree_cover_ttl) ~ TDI + yst + yst:TDI + (1|scode) + (1|subplot_id), data = tcover)
summary(m)
lincon(m)
...
qqnorm(resid(m)); qqline(resid(m))
ggplot(m, aes(x = .fitted, y = .resid)) + geom_point() + facet_wrap(~yst) + scale_x_continuous(limits = c(0,1))

#by yst and site
tcover$yhat_tree_cover <- predict(m)
p <- ggplot(data = tcover, aes(x = TDI, y = sqrt(tree_cover_ttl), color = scode))
p <- p + geom_jitter()
p <- p + geom_line(aes(y = yhat_tree_cover))
p <- p + theme_bw()
p <- p + labs(title = 'Tree Cover',
