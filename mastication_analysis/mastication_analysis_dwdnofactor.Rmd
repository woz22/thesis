...
```{r}
m <- lmer(sqrt(herb_ttl) ~ TDI + yst + yst:TDI + (1 + yst|scode), data = l)
...
p <- ggplot(data = l, aes(x = TDI, y = sqrt(herb_ttl), color = scode))
p <- p + geom_point()
p <- p + geom_line(aes(y = yhat_herb))
p <- p + theme_bw()
p <- p + labs(title = 'Herbaceous Fuels',
...
p <- ggplot(data = l, aes(x = TDI, y = sqrt(shrub_fuel)))
p <- p + geom_point()
...
p <- ggplot(data = tcover, aes(x = TC, y = sqrt(tree_cover_ttl)))
p <- p + geom_jitter()
